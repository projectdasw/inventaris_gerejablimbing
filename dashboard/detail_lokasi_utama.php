...
<div class="bg-light overflow-x-auto">
    <table id="" class="display table table-hover">
        ...
        <tbody>
            <?php
                $get_id = $_GET['detail_lokasi_utama'];

                // GET DATA LOKASI UTAMA w/ URL
                $query_get = "select * from lokasi_utama where id_lokasi = '$get_id'";
                $sql_get = mysqli_query($connect, $query_get);
                $view_data = mysqli_fetch_assoc($sql_get);
                $nm_loc = $view_data['nama_lokasi_utama'];
                // GET DATA LOKASI UTAMA w/ URL

                $query = "select * from barang_gereja where nama_lokasi_utama = '$nm_loc'";

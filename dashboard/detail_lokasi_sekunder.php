...
<div class="bg-light overflow-x-auto">
    <table id="" class="display table table-hover">
        ...
        <tbody>
            <?php
                $get_id = $_GET['detail_lokasi_sekunder'];

                // GET DATA LOKASI SEKUNDER w/ URL
                $query_get = "select * from lokasi_sekunder where id_lokasi_sekunder = '$get_id'";
                $sql_get = mysqli_query($connect, $query_get);
                $view_data = mysqli_fetch_assoc($sql_get);
                $nm_los = $view_data['nama_lokasi_sekunder'];
                // GET DATA LOKASI SEKUNDER w/ URL

                $query = "select * from barang_gereja where nama_lokasi_sekunder = '$nm_los'";
